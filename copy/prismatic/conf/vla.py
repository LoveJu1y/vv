...
class VLAConfig(ChoiceRegistry):
    ...
    # Implicit Reasoning Training Parameters (for train_latent function)
    latent: bool = False                            # Enable implicit reasoning training mode
    epochs_per_stage: int = 1                       # Number of epochs per training stage (only effective when latent=True)
    thinking_token_count: int = 2                   # Number of thinking tokens per stage (default 5)
    thinking_token: str = "<|thinking|>"              # Single thinking token for implicit reasoning
    start_of_thinking_token: str = "<|start_of_thinking|>"    # Start of thinking token for implicit reasoning
    end_of_thinking_token: str = "<|end_of_thinking|>"      # End of thinking token for implicit reasoning
    tag2think_count: Dict[str, int] = field(default_factory=lambda: {
        "TASK": 1,
        "PLAN": 1,
        "VISIBLE OBJECTS": 1,
        "SUBTASK REASONING": 1,
        ...
        "MOVE": 1,
        "GRIPPER POSITION": 1,
    })# tag2think_count: Dict[str,int] = {"TASK": 1, "PLAN": 1, "VISIBLE_OBJECTS": 2, "SUBTASK_REASONING": 2, "SUBTASK": 1, "MOVE_REASONING": 1, "MOVE": 1, "GRIPPER_POSITION": 1} # Number of thinking tokens per stage (default 5)
    
    def __post_init__(self):
        """Initialize default thinking token if not provided"""
        if self.thinking_token is None:
            self.thinking_token = "<thinking>"
        if self.start_of_thinking_token is None:
            self.start_of_thinking_token = "<start_of_thinking>"
        if self.end_of_thinking_token is None:
            self.end_of_thinking_token = "<end_of_thinking>"
...
# @dataclass
# class Exp_SigLIP_224px_Bridge_single_gpu(VLAConfig):
#     vla_id: str = "siglip-224px+mx-bridge-single-gpu"
#     base_vlm: Union[str, Path] = "siglip-224px+7b"
#     freeze_vision_backbone: bool = False
#     freeze_llm_backbone: bool = False
#     unfreeze_last_llm_layer: bool = False
...
#single gpu debugging
@dataclass
class Exp_DinoSigLIP_224px_Bridge_single_gpu(Exp_SigLIP_224px_Bridge):
    vla_id: str = "prism-dinosiglip-224px+mx-bridge-single-gpu"
    base_vlm: Union[str, Path] = "prism-dinosiglip-224px+7b"
    data_mix: str = "bridge"
    expected_world_size: int = 1
    global_batch_size: int = 2
...
class VLARegistry(Enum):
    ...
    # === Single GPU Debugging Configs ===
    DINOSIGLIP_224PX_MX_BRIDGE_SINGLE_GPU = Exp_DinoSigLIP_224px_Bridge_single_gpu
