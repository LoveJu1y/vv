...
from prismatic.util.cot_utils import get_cot_database_keys_simple, get_cot_tags_list_simple
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    reasoning_dataset_path: str = "/share/project/lvjing/datas/embodied_features_bridge.json",
    ...
    def make_tf_dict(raw_dict, scheduled_stage=0, cfg=None):
        print(f"Building the reasoning dict for stage {scheduled_stage}...")
        keys = []
        values = []

        def reasoning_dict_to_str(d, stage=scheduled_stage):
            
            
            # Get all CoT tags except ACTION
            all_tags = get_cot_tags_list_simple()
            database_keys = get_cot_database_keys_simple()
            
            # Determine which tags to include based on stage
            if stage == 0:
                # Stage 0: Include all CoT components
                active_tags = all_tags
            else:
                # For stage > 0, remove the first (stage-1) components
                active_tags = all_tags[stage:] if stage < len(all_tags) else []
            
            reasoning_parts = []
            if cfg is not None and hasattr(cfg, 'vla'):
                thinking_token_count = getattr(cfg.vla, 'thinking_token_count', 1)  # default 2 thinking tokens
                tag2think_count = getattr(cfg.vla, 'tag2think_count', None)
                start_token = getattr(cfg.vla, 'start_of_thinking_token', '<|start_of_thinking|>')
                thinking_token = getattr(cfg.vla, 'thinking_token', '<|thinking|>')
                end_token = getattr(cfg.vla, 'end_of_thinking_token', '<|end_of_thinking|>')
            else:
                tag2think_count = {1: 1, 2: 1, 3:2, 4:2, 5:1, 6:1, 7:1, 8:1}
                start_token = '<|start_of_thinking|>'
                thinking_token = '<|thinking|>'
                end_token = '<|end_of_thinking|>'
            
            # Collect all thinking tokens first
            thinking_tokens = []
            active_parts = []
            
            # Process all tags
            removed_count = 0
            total_removed = len(all_tags) - len(active_tags)
            # assert len(all_tags) == len(database_keys), f"all_tags: {len(all_tags)}, database_keys: {len(database_keys)}"
            
                # return ''
            for i, tag in enumerate(all_tags):
                if tag in active_tags :
                    # Include this component
                    active_parts.append((tag, d[database_keys[tag]]))
                else:
                    # Add thinking tokens to the collection
                    if total_removed == 1:
                        # Only one component removed: start + thinking + end
                        thinking_tokens.extend([start_token] + [thinking_token] * tag2think_count[tag.replace(":", "").upper()] + [end_token])
                    else:
                        # Multiple components removed
                        if removed_count == 0:  # First removed component
                            thinking_tokens.extend([start_token] + [thinking_token] * tag2think_count[tag.replace(":", "").upper()])
                        elif removed_count == total_removed - 1:  # Last removed component
                            thinking_tokens.extend([thinking_token] * tag2think_count[tag.replace(":", "").upper()] + [end_token])
                        else:  # Middle removed components
                            thinking_tokens.extend([thinking_token] * tag2think_count[tag.replace(":", "").upper()])
                    removed_count += 1
            
            # Combine all parts: thinking tokens first, then active parts
            all_parts = []
            if thinking_tokens:
                # IMPORTANT: Thinking tokens should be joined WITHOUT spaces between them
                # to avoid extra space tokens (220) during tokenization
                # Example: "<|start_of_thinking|><|thinking|><|thinking|><|end_of_thinking|>"
                all_parts.append(('', "".join(thinking_tokens)))
            all_parts.extend(active_parts)
            
            # Format without @ delimiter: use space separation with TAG markers
            # If tag is empty (thinking tokens), use part directly; otherwise use "tag part"
            # Note: Space is added between parts, but NOT within thinking tokens
            return " ".join(
                part if not tag else f"{tag} {part}"
                for tag, part in all_parts
            )
        ...
        for file_name in raw_dict.keys():
            for episode_id in raw_dict[file_name].keys():
                ...
                for i in raw_dict[file_name][episode_id]["reasoning"].keys():
                    ...
                    try:
                        temp = reasoning_dict_to_str(reasoning_dict)
                    except Exception as e:
                        print(f"Error in reasoning_dict_to_str: {e}")
    ...
    reasoning_dataset = make_tf_dict(reasoning_dataset, scheduled_stage, cfg)
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    # Filter out samples with empty reasoning (for latent reasoning training)
    if "reasoning" in dataset.element_spec:
        dataset = dataset.filter(lambda x: tf.math.reduce_any(x["reasoning"] != ""))
    ...
    *,
    ...
    Args:
        ...
        traj_read_threads: total number of parallel read workers for trajectory transforms, distributed across
            datasets according to their sampling weights. If None, defaults to AUTOTUNE for every dataset.
        scheduled_stage: the stage of the dataset to use.
    """
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        _, dataset_statistics = make_dataset_from_rlds(**data_kwargs, train=train, scheduled_stage=scheduled_stage, cfg=cfg)
