...
def normalize_action_and_proprio(traj: Dict, metadata: Dict, normalization_type: NormalizationType):
    ...
    if normalization_type == NormalizationType.NORMAL:
        for key, traj_key in keys_to_normalize.items():
            ...
            mean = tf.convert_to_tensor(metadata[key]["mean"], dtype=tf.float32)
            std = tf.convert_to_tensor(metadata[key]["std"], dtype=tf.float32)

            mask_raw = metadata[key].get("mask", tf.ones_like(mean, dtype=tf.bool))
            mask = tf.convert_to_tensor(mask_raw, dtype=tf.bool)

            traj = dl.transforms.selective_tree_map(
                traj,
                match=lambda k, _: k == traj_key,
                map_fn=lambda x: tf.where(mask, (x - mean) / (std + 1e-8), x),
            )
        ...
        for key, traj_key in keys_to_normalize.items():
            ...
            # choose low/high
            if normalization_type == NormalizationType.BOUNDS:
                low_raw = metadata[key]["min"]
                high_raw = metadata[key]["max"]
            else:  # BOUNDS_Q99
                low_raw = metadata[key]["q01"]
                high_raw = metadata[key]["q99"]

            # ✅ convert to Tensor
            low = tf.convert_to_tensor(low_raw, dtype=tf.float32)
            high = tf.convert_to_tensor(high_raw, dtype=tf.float32)

            mask_raw = metadata[key].get("mask", tf.ones_like(low, dtype=tf.bool))
            mask = tf.convert_to_tensor(mask_raw, dtype=tf.bool)
            ...
            # unused dim where min == max → zero mask
            zeros_mask_raw = np.equal(metadata[key]["min"], metadata[key]["max"])
            zeros_mask = tf.convert_to_tensor(zeros_mask_raw, dtype=tf.bool)
...
def get_dataset_statistics(
    ...
    if save_dir is not None:
        ...
        path = local_path
    path = "/share/project/emllm_mnt.1d/mnt/sfs/baaiei/jyShi/rt_newData/bridge_data_v2/0.0.1/dataset_statistics_929e6c2a379da93d098d1811174d857c23156cd6bebcf7a76e5d2b80eb2500e4.json"#dataset_statistics_e058f24e49956a487beb4ddf679b6a736a7b2b1ffa2a4f71ca1e2aeb960ecac3.json"
