...
def load_vla(
    ...
    if cfg.latent_reason and cfg.resume_latent:
        overwatch.info("Adding thinking tokens to tokenizer for implicit reasoning training")
        tokenizer = llm_backbone.get_tokenizer()
        tokenizer.add_tokens(cfg.vla.thinking_token)
        tokenizer.add_tokens(cfg.vla.start_of_thinking_token)
        tokenizer.add_tokens(cfg.vla.end_of_thinking_token)
        
        # Resize model embeddings and initialize new token embeddings for training stability
        overwatch.info("Resizing model embeddings and initializing new token embeddings")
        # Use the VLM's method to resize embeddings instead of direct access
        llm_backbone.llm.resize_token_embeddings(len(tokenizer))
        print("resize_token_embeddings",len(tokenizer))
        # Initialize new token embeddings with a known token for training stability
        embeddings = llm_backbone.llm.get_input_embeddings()
        target_id = tokenizer.convert_tokens_to_ids("<<")
        
        # Get token IDs for the new tokens
        thinking_token_id = tokenizer.convert_tokens_to_ids(cfg.vla.thinking_token)
        start_thinking_id = tokenizer.convert_tokens_to_ids(cfg.vla.start_of_thinking_token)
        end_thinking_id = tokenizer.convert_tokens_to_ids(cfg.vla.end_of_thinking_token)
        
        # Initialize embeddings with target token embedding
        for token_id in [thinking_token_id, start_thinking_id, end_thinking_id]:
            if token_id != tokenizer.unk_token_id:  # Make sure token was successfully added
                target_embedding = embeddings.weight.data[target_id]
                embeddings.weight.data[token_id] = target_embedding
                
                # Also initialize LM head weights (for models where they're not tied)
                if hasattr(llm_backbone.llm, 'lm_head') and llm_backbone.llm.lm_head is not None:
                    lm_head = llm_backbone.llm.lm_head
                    lm_head.weight.data[token_id] = lm_head.weight.data[target_id]
        
        overwatch.info(f"Added thinking tokens: {cfg.vla.thinking_token}, {cfg.vla.start_of_thinking_token}, {cfg.vla.end_of_thinking_token}")
        overwatch.info("Initialized new token embeddings for training stability")
    else:
        overwatch.info("Latent reasoning training mode disabled, using standard CoT training")
...
def load_ecot(
    ...
    vla = AutoModelForVision2Seq.from_pretrained("/share/project/lvjing/embodied-CoT/ecot.pt", torch_dtype=torch.bfloat16, trust_remote_code=True)
